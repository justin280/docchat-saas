    ...
    <div style={{backgroundColor:'#0a0a0a',minHeight:'100vh',color:'#fff',fontFamily:'system-ui,sans-serif'}}>
      ...
      <section style={{textAlign:'center',padding:'64px 20px 48px',maxWidth:'860px',margin:'0 auto'}}>
        ...
        <div style={{marginBottom:'20px'}}>
          <span style={{backgroundColor:'#84cc16',color:'#000',padding:'5px 14px',borderRadius:'20px',fontSize:'11px',fontWeight:'700',letterSpacing:'1px',textTransform:'uppercase'}}>Enterprise Security Pack</span>
        </div>
              ...
              <ul style={{listStyle:'none',padding:0,margin:0,display:'flex',flexDirection:'column',gap:'8px'}}>
                {doc.sections.map((s,i) => (
                  <li key={i} style={{display:'flex',alignItems:'center',gap:'8px',color:'#d1d5db',fontSize:'13px'}}>
                    <span style={{color:'#84cc16',fontSize:'12px',flexShrink:0}}>&#10003;</span>
                    {s}
                  </li>
                ))}
              </ul>
      ...
      <section style={{maxWidth:'700px',margin:'0 auto 80px',padding:'0 20px',textAlign:'center'}}>
        <div style={{backgroundColor:'#111',border:'1px solid #222',borderRadius:'12px',padding:'28px'}}>
          <p style={{color:'#fff',fontSize:'15px',fontWeight:'700',margin:'0 0 8px'}}>Need signed BAA, custom security assessment, or architecture review?</p>
          <p style={{color:'#6b7280',fontSize:'13px',margin:'0 0 20px',lineHeight:1.6}}>These templates are ready to use. For executed agreements or bespoke documentation, contact the security team.</p>
          <Link href="/contact" style={{backgroundColor:'#84cc16',color:'#000',padding:'11px 28px',borderRadius:'8px',fontWeight:'700',fontSize:'14px',textDecoration:'none',display:'inline-block'}}>Contact Security Team</Link>
        </div>
      </section>

      {/* Footer */}
      <footer style={{borderTop:'1px solid #111',padding:'24px 20px',textAlign:'center',color:'#6b7280',fontSize:'13px'}}>
        <p style={{margin:0}}>&#169; {new Date().getFullYear()} DocChat AI &#8212; <Link href="/security" style={{color:'#9ca3af',textDecoration:'none'}}>Security</Link> &#183; <Link href="/contact" style={{color:'#9ca3af',textDecoration:'none'}}>Contact</Link> &#183; <Link href="/" style={{color:'#9ca3af',textDecoration:'none'}}>Home</Link></p>
      </footer>
    </div>
  );
}
