...
const TEMPLATES = [
  { icon: 'ð', label: 'Summarise', prompt: 'Please provide a concise summary of this document, covering the main points and key takeaways.' },
  { icon: 'ð¯', label: 'Key Points', prompt: 'Extract the most important key points from this document as a numbered list.' },
  { icon: 'â', label: 'Action Items', prompt: 'List all action items, tasks, or next steps mentioned in this document.' },
  { icon: 'ð', label: 'Key Dates', prompt: 'Extract all dates, deadlines, and time-sensitive information from this document.' },
  { icon: 'â ï¸', label: 'Risks', prompt: 'Identify any risks, concerns, warnings, or potential issues mentioned in this document.' },
  { icon: 'â', label: 'FAQ', prompt: 'Generate a list of 5 frequently asked questions and answers based on this document.' },
];
const ACCEPTED = '.pdf,.docx,.txt,.xlsx,.csv,.md,.html,.htm,.rtf,.epub,.odt';
const MAX_DOCS = 5;
const USE_CASES = [
  { icon: 'âï¸', title: 'Legal', desc: 'Upload contracts, find clauses instantly.' },
  { icon: 'ð¥', title: 'Healthcare', desc: 'Query research papers and reports.' },
  { icon: 'ð', title: 'Finance', desc: 'Ask about annual reports and statements.' },
  { icon: 'ð', title: 'Students', desc: 'Upload notes, get answers for essays.' },
  { icon: 'ð¢', title: 'HR', desc: 'Search handbooks and policy documents.' },
  { icon: 'ð¬', title: 'Research', desc: 'Ask across multiple academic papers.' },
];
const FAQS = [
  { q: 'What file types are supported?', a: 'PDF, DOCX, TXT, XLSX, CSV, Markdown, HTML, RTF, EPUB, and ODT.' },
  { q: 'Can I upload multiple documents?', a: 'Yes â upload up to 5 documents and chat across all of them at once.' },
...
export default function Home() {
  ...
  const processFile = async (f) => {
    ...
    if (data.text) {
      ...
      setDocuments(prev => {
        ...
        setMessages(msgs => [...msgs, { role: 'assistant', content: updated.length === 1 ? `ð **${f.name}** loaded! Ask me anything about it.` : `ð **${f.name}** added. Chatting across ${updated.length} documents.` }]);
        return updated;
      });
  ...
  const exportChat = () => {
    const lines = ['DocChat AI â Conversation Export', '='.repeat(40), ''];
    ...
    <main className="min-h-screen flex flex-col" style={{background:'#0f0f0f'}}>
      <div className="flex items-center justify-between px-4 py-3 border-b" style={{borderColor:'#2a2a2a',background:'#111'}}>
        ...
        <div className="flex items-center gap-2">
          ...
          {messages.length > 0 && <button onClick={exportChat} className="text-xs px-3 py-1.5 rounded-lg" style={{background:'#1a1a1a',color:'#888',border:'1px solid #2a2a2a'}}>â¬ Export</button>}
          <button onClick={() => setView('landing')} className="text-xs text-gray-500 hover:text-gray-300">â Plans</button>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2">
            ...
            {documents.map(doc => (
              <div key={doc.id} className="flex items-start gap-2 p-2 rounded-lg group" style={{background:'#1a1a1a',border:'1px solid #2a2a2a'}}>
                <span className="text-sm mt-0.5">{doc.name.endsWith('.pdf')?'ð':doc.name.endsWith('.xlsx')||doc.name.endsWith('.csv')?'ð':'ð'}</span>
                ...
                <button onClick={()=>removeDoc(doc.id)} className="text-gray-700 hover:text-red-400 text-xs opacity-0 group-hover:opacity-100">â</button>
              </div>
            ))}
          </div>
  ...
  return (
    <main className="min-h-screen flex flex-col items-center px-4 py-12" style={{background:'#0f0f0f'}}>
      <div className="text-center mb-16 max-w-2xl">
        ...
        <p className="text-gray-400 mb-4">Upload up to 5 documents in any format â PDF, DOCX, XLSX, TXT, Markdown and more â then ask questions across all of them.</p>
        ...
        <button onClick={()=>handleSubscribe('free')} className="px-8 py-4 rounded-xl font-bold text-lg" style={{background:'#76b900',color:'#000'}}>Try Free â No Credit Card Needed</button>
        <p className="text-gray-600 text-xs mt-3">Powered by NVIDIA NIM Â· Llama Â· Mistral Â· DeepSeek</p>
      </div>
      <div className="w-full max-w-4xl mb-16 grid grid-cols-3 gap-4">
        {[{icon:'ð',title:'Multi-Document Chat',desc:'Upload up to 5 documents, ask across all simultaneously'},{icon:'ð¤',title:'3 AI Models',desc:'Switch between Llama 3.1, Mistral Large, DeepSeek R1 mid-chat'},{icon:'â¡',title:'Quick Prompts',desc:'One-click: Summarise, Key Points, Action Items, Risks, Dates'},{icon:'ð',title:'Spreadsheet Support',desc:'Upload Excel and CSV, ask questions about your data'},{icon:'â¬ï¸',title:'Export Chat',desc:'Download your full conversation as a text file'},{icon:'ð',title:'Private & Secure',desc:'Never stored. Processed in real-time. Your data stays yours.'}].map(f=>(
        ...
        <div className="grid grid-cols-3 gap-6">
          {PLANS.map(p=>(
            <div key={p.id} className="rounded-2xl p-6 flex flex-col" style={{background:p.highlight?'#0d1a00':'#1a1a1a',border:p.highlight?'2px solid #76b900':'1px solid #2a2a2a'}}>
              ...
              <ul className="space-y-2 mb-6 flex-1">{p.features.map(f=><li key={f} className="flex items-center gap-2 text-xs text-gray-300"><span style={{color:'#76b900'}}>â</span>{f}</li>)}</ul>
